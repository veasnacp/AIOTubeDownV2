...
class TikTokBaseIE:
  ...
  def get_user_id(self, url_uid:str):
    user_id = url_uid.split("@")[1].split('?')[0].split('/')[0] if "@" in url_uid else url_uid.strip()
    return user_id
  ...
  def extract_node(self, node:dict, only_url_dl=False) -> dict[str,any]:
    ...
    music = node.get("music") if isinstance(node.get("music"), dict) else node.get("music_info", {})
    _video = video.get("video")
    _stats = video.get("stats",{}) if isinstance(video.get("stats"), dict) else video
    stats = {
    ...
    if isinstance(_video, dict):
      ...
      playwm = video["playwm"] if video.get("playwm") else video.get("wmplay","")
      play_addr = video["play_addr"] if video.get("play_addr") else video.get("play","")
      url_dl = play_addr
    ...
    user_keys = ["id","nickname","avatar", "avatarLarger","avatarMedium","avatarThumb","signature","secUid"]
    ...
    title = node["title"] if node.get("title") is not None else node.get("desc","")
    title = title if title != "" else "Video by %s [%s]" % (user_id, video_id)

    if isinstance(_video, dict):
      cover = _video.get("cover","")
      origin_cover = _video.get("originCover",cover)
    else:
      cover = video["cover_original_scale"] if video.get("cover_original_scale") is not None else video.get("cover","")
      origin_cover = video["origin_cover"] if video.get("origin_cover") is not None else cover
...
class TikTokRequest(TikTokBaseIE):
  ...
  async def extract_video_list_from_other(self, url_list, only_url_dl=False):
    ...
    for i, content in enumerate(data_list):
      ...
      try:
        ...
        data_content = get_content_from_html_selector(str(content), "script", ['id=\"__UNIVERSAL_DATA_FOR_REHYDRATION__\"'])
        if len(data_content) > 0:
          ...
          if isinstance(node, dict) and str(node.get("createTime")) and int(node.get("createTime")) != 0:
            ...
            print("Error => Create Time is 0 with URL: ", url_list[i])
  ...
  def extract_video_list_from_other_pool(self, video_url_list, only_url_dl=False):
    ...
    for url_list in video_url_list_of_list:
      ...
      cpu = use_cpu(runtime)
      with Pool(cpu) as p:
        video_info_list_of_list_ = p.starmap(self.extract_video_list_from_other_run, zip(*[url_list_of_list, only_url_dl_list]))
        p.close()
        p.join()
  ...
  def extract_video_info_list_mobile_api_sln_all(
    ...
    while True:
      ...
      try:
        data_list = browser.execute_script(execute_script_promise_all(url_list))
        if len(url_list) > 20:
          time.sleep(5)
...
class TikTokExtractor(TikTokRequest):
  ...
  def extract_video_info(self, url_vid:str, only_url_dl=False):
    try:
      video_info = self._extract_node_yt_dlp(self.get_url_vid(url_vid), True)
      if only_url_dl is True:
  ...
  async def extract_video_info_list(self, url_vid_list:list[str], only_url_dl=False):
    ...
    def get_video_info_list(get_tasks):
      ...
      for video_info in get_tasks:
        if isinstance(video_info, dict):
          is_error = isinstance(video_info.get("status_code"), int) and video_info.get("url")
          if is_error:
            url = unquote(video_info.get("url","?url=").split("url=")[1].split("&")[0])
    ...
    video_info_list_1, video_list_1, error_video_list = get_video_info_list(get_tasks)
    ...
    if len(error_video_list) > 0:
      ...
      video_info_list_2, video_list_2, error_video_list_2 = get_video_info_list(get_tasks_2)
  ...
  async def extract_video_info_list_tikwm(self, url_vid_list:list[str], only_url_dl=False) -> list[dict]:
    ...
    api_url_list = [self._API_TIKWM_VIDEO % self.get_url_vid(url_vid) for url_vid in url_vid_list]
    # print(api_url_list)
  ...
  async def extract_info_profile(
    ...
    for i in range(300):
      ...
      url = self._API_TIKWM_USER % (
        user_id,
        limit if (isinstance(limit, int) and limit != 0) and limit < 35 else 35,
        cursor
      )
    ...
    if only_url_dl is True:
      video_list = [self.dict_to_url_quote(info)['url_dl'] for info in video_info_list if isinstance(info, dict) and info.get('hd')]
      return video_list
      ...
      async def extract_videos_from_user(url_username):
        ...
        if self.server == "1" or self.server == "default":
          ...
          try:
            ...
            profile_video_info = await self.extract_info_profile_pwr(url_username,limit,sort_by)
          if only_url_dl:
            video_list = profile_video_info.get("video_list") #type: list
          else:
            video_list = profile_video_info.get("video_info_list") #type: list

        await asyncio.sleep(0.0001)
        # print("[[[Video list]]]: ", json.dumps(video_list), "\n")
        return video_list

      tasks = [extract_videos_from_user(url) for url in url_username_list]
      get_tasks = await asyncio.gather(*tasks, return_exceptions=True)
      return [
