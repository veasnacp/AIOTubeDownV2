...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QApplication, QSpacerItem, QSizePolicy
from PySide6Addons import (TransparentToolButton, FluentIcon, InfoBar, 
                             SubtitleLabel, BodyLabel, CaptionLabel, 
                             SimpleCardWidget, LineEdit, PrimaryPushButton, 
                             PushButton, setFont)
import sys
...
class LicenseInterface(QWidget):
    ...
    def _copy_hw_id(self):
        ...
        InfoBar.success("Success", "Hardware ID copied to clipboard", duration=2000, parent=self)
    ...
    def _on_activate_clicked(self):
        ...
        if license_manager.activate(key):
            self._update_status_ui()
            InfoBar.success("Congratulations", "Application successfully activated!", duration=4000, parent=self)
            self.key_edit.clear()
        else:
            InfoBar.error("Invalid Key", "The license key provided is invalid for this machine.", duration=4000, parent=self)
